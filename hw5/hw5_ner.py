...
class NamedEntityRecognitionMEMM:
    ...
    def generate_probabilities(self, data):
        ...
        # these are the lists of dics we will return
        dics_probs = []
        dics_ptrs = []
        # this stores the tag for the previous iteration
        prev_tag = None
        # this stores the word for the previous iteration
        prev_word = None
        # iterate through our data
        for i in range(len(data)):
            # this stores the next word in the data
            next_word = None
            # this is our current word
            word = data[i]
            # if there is space to get another token then get it
            if i <= (len(data) - 2):
                next_word = data[i+1]
            # get the features for our data
            features = self.featurize(prev_tag, prev_word, word, next_word)
            # this is where we will store our probabilities
            y_hat = {}
            # this is the maximum and the state of the maximum we will store
            max = -1
            max_state = None
            # do the dot product of our weights and features for every state
            for state in self.states:
                y_hat[state] = np.dot(self.weights[state], features)
            # store the denominator for our softmax function
            denom = sum(np.exp(np.array(list(y_hat.values()))))
            # run the softmax function and store the max and what state the max was
            for state in self.states:
                y_hat[state] = np.exp(y_hat[state]) / denom
                # if it is better than the max then store the its prob and state
                if y_hat[state] > max:
                    max = y_hat[state]
                    max_state = state
            # our back ptrs all come from the same place which was the maximum of the last tag so make that dictionary
            ptrs = {'I': prev_tag, 'O': prev_tag, 'B': prev_tag}
            # append our two new dictionariesto the list of dictionaries
            dics_probs.append(y_hat)
            dics_ptrs.append(ptrs)
            # store our prev_tag and prev_words as the current for the next iteration
            prev_tag = max_state
            prev_word = word
        # return our dictionaries
...
if __name__ == "__main__":
    ...
    HMM = NamedEntityRecognitionHMM()
    HMM.train(training_examples)
    data = []
    ...
    for sentence in data:
        probs, ptrs = HMM.generate_probabilities(sentence)
        labels.append(decode(sentence, probs, ptrs))

    p = precision(testing_examples, labels)
    r = recall(testing_examples, labels)
    f1_val = f1(testing_examples, labels)
    print('precision:', p)
    print('recall:', r)
    print('f1:', f1_val)

    MEMM = NamedEntityRecognitionMEMM
    ...
    labels = []
    for sentence in data:
        probs, ptrs = MEMM.generate_probabilities(sentence)
        labels.append(decode(sentence, probs, ptrs))
    p = precision(testing_examples, labels)
    r = recall(testing_examples, labels)
    f1_val = f1(testing_examples, labels)
